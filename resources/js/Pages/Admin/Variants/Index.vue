    ...
    export default {
        ...
        methods: {
            ...
            actionHandler(item,type){
                if(type == "edit"){
                    this.$inertia.visit(this.route('admin.variants.edit',item.id));
                }else if(type == "delete"){
                    this.confirmBoxForRecordDeletion(this.route('admin.variants.destroy',{variant: item.id}));
                }
            }
        }
    }
...
<template>
    ...
    <BackendLayout :flash="flash">
        <div class="rt-head-wrap">
            <div class="heading">Variants</div>
            <a :href="route('admin.variants.create')" class="btn-add">Add Variant</a>
        </div>
        <v-data-table-server
