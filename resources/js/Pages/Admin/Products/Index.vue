    ...
    export default {
        ...
        data(){
            return {
                ...
                headers: [
                    ...
                    {title: 'Catalog Number',key: 'catalog_number',sortable: false},
                    {title: 'CAS Number',key: 'cas_number',sortable: false},
                    {title: 'Product Type',key: 'product_type',sortable: false},
        ...
        methods: {
            ...
            actionHandler(item,type){
                if(type == "edit"){
                    this.$inertia.visit(this.route('admin.products.edit',item.id));
                }else if(type == "delete"){
                    this.confirmBoxForRecordDeletion(this.route('admin.products.destroy',{product: item.id}));
                }
            }
        }
    }
            ...
            <template v-slot:item="{item}">
                <tr>
                    <td style="padding:10px 16px;">
                        <div class="rt-produst-wrap">
                            <img :src="item.image" :alt="item.name" class="product-img"/>
                            <div class="product-info">
                                <div class="rt-table-text">{{ item.name }}</div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td>{{(item.price ? (item.price).toLocaleString('en-US',{style:'currency',currency:'USD'}) : null)}}</td>
                    <td>
